...
import (
	...
	"github.com/cmliang/weapp/v3/encrypt"
...
func anyToJsonArray(vAny, list any) error {
	...
	var vList []interface{}
	if jr.IsObject() {
		vList = append(vList, jr.Value())
	} else if jr.IsArray() {
		vv, ok := jr.Value().([]interface{})
		if ok {
			vList = vv
		}
	}
	bt, err := json.Marshal(&vList)
	if err != nil {
		return err
	} else {
		return json.Unmarshal(bt, &list)
	}
}
...
func (srv *Server) handleRequest(w http.ResponseWriter, r *http.Request, isEncrpt bool, ctp request.ContentType) (interface{}, error) {
	...
	// 默认使用通用handler，仅当各个过滤器的handler已注册且未return时变为false
	...
	case MsgEvent:
		...
		case EventSubscribeMsgSentEvent:
			...
			if srv.subscribeMsgSentHandler != nil {
